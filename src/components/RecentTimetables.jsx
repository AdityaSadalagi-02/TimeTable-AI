...
const RecentTimetables = () => {
  ...
  const fetchHistory = async () => {
    ...
    if (error) { toast.error("Could not load timetables"); console.error(error); }
  ...
  const handleDelete = async (item) => {
    if (!window.confirm(
      `Delete timetable for ${item.department} Sem ${item.semester} (${formatDate(item.created_at)})?`
    )) return;
    ...
    try {
      ...
      // ── 1. Build the set of (teacher_id, day, slot) to remove ────────────
      // Fetch teacher_subjects to map subject_name → teacher_id
      const { data: teacherLinks } = await supabase
        .from("teacher_subjects")
        .select("teacher_id, subjects(subject_name)");
      ...
      // Also handle combined lab label (e.g. "WEB LAB/CN LAB")
      // Any value containing "/" could be a combined lab label — map it too
      Object.keys(subjectTeacherMap).forEach((name) => {
        // Already individual names mapped above
      });

      // Collect every (teacher_id, day_of_week, time_slot) used in this timetable
      const slotsToFree = [];
      for (const day in tt) {
        for (const slot in tt[day]) {
          const subject = tt[day][slot];
          if (!subject || subject === "-") continue;

          // Handle combined lab label — try matching each part
          const parts = subject.includes("/") ? subject.split("/") : [subject];
          parts.forEach((part) => {
            const teacherId = subjectTeacherMap[part.trim()];
            if (teacherId) {
              slotsToFree.push({ teacher_id: teacherId, day_of_week: day, time_slot: slot });
            }
          });
        }
      }

      // ── 2. Delete teacher_availability rows for this timetable's slots ────
      // We delete only slots that match BOTH teacher_id AND semester_id
      // so other semesters' busy slots for the same teacher are untouched
      const semesterId = item.semester;
      for (const row of slotsToFree) {
        await supabase
      ...
      const { error } = await supabase.from("saved_timetables").delete().eq("id", item.id);
  ...
  const handleDownload = async (item) => {
    ...
    try {
      ...
      const classSlotKeys = [...Object.keys(tt[firstDay] || {})]
      ...
      doc.text(`${item.department} — Semester ${item.semester} Timetable`, 14, 16);
      doc.setFontSize(9);
      ...
      // Build head row
      const headRow = ["Day", ...columns.map((c) => c.type !== "class" ? c.label : c.label)];
      const head = [headRow];
      ...
      const colorMap = {};
      const hexPalette = [
        [219,234,254],[220,252,231],[254,249,195],[252,231,243],[237,233,254],
        [255,237,213],[224,242,254],[209,250,229],[254,243,199],[243,232,255],
      ];
      let ci = 0;
      body.forEach((row) => row.slice(1).forEach((val) => {
        if (val && val !== "—" && val !== "BREAK" && val !== "LUNCH" && !colorMap[val])
      ...
      autoTable(doc, {
        ...
        startY: 28,
        styles: {
          fontSize: 7.5, cellPadding: 3, halign: "center", valign: "middle",
          lineColor: [226, 232, 240], lineWidth: 0.3, overflow: "linebreak", minCellHeight: 12,
        },
        headStyles: {
          fillColor: [241, 245, 249], textColor: [30, 41, 59], fontStyle: "bold", fontSize: 7,
        },
        columnStyles: {
          0: { fontStyle: "bold", fillColor: [248, 250, 252], halign: "left", cellWidth: 22 },
        },
        didParseCell: (data) => {
  ...
  const detectLabLabels = (tt) => {
    ...
    DAYS.forEach((day) => {
      const keys = Object.keys(tt[day] || {}).sort((a, b) => parseSlotStart(a) - parseSlotStart(b));
  ...
  const TimetableModal = ({ item, onClose }) => {
    ...
    const buildRow = (day) => {
      ...
      for (let i = 0; i < columns.length; i++) {
        ...
        cells.push({ colType: "class", label: col.label, subject, span, isLab });
      }
      return cells;
    };
      ...
      <div style={modal.overlay} onClick={onClose}>
        <div style={modal.box} onClick={(e) => e.stopPropagation()}>
          ...
          <div style={modal.header}>
            <div>
              <h2 style={{ margin: 0 }}>📅 {item.department} — Semester {item.semester}</h2>
              ...
              <thead>
                <tr>
                  ...
                  {columns.map((col, idx) => {
                    if (col.type === "break" || col.type === "lunch") {
                      return (
                        <th key={idx} style={{ ...tStyle.th, background: "#fde68a", color: "#92400e", minWidth: 60 }}>
                          {col.label}
                        </th>
                      );
                    }
                    return <th key={idx} style={tStyle.th}>{col.label}</th>;
                  })}
                </tr>
              </thead>
              <tbody>
                {DAYS.map((day) => (
                  <tr key={day}>
                    ...
                    {buildRow(day).map((cell, idx) => {
                      // Break / Lunch cell
                      if (cell.colType === "break" || cell.colType === "lunch") {
                        return (
                          <td key={idx} style={tStyle.breakCell}>{cell.label}</td>
                        );
                      }
                      // Lab cell (colspan)
                      if (cell.isLab) {
                        return (
                          <td key={idx} colSpan={cell.span} style={{
                            ...tStyle.labCell,
  ...
  return (
    <div className="card">
      <div className="card-header">
        ...
        <p>View, download or delete previously generated schedules.</p>
      </div>
            ...
            {history.map((item) => (
              <div key={item.id} style={listCard}>
                <div style={{ display: "flex", alignItems: "center", gap: 16, flex: 1 }}>
                  <div style={deptBadge}>
                    <span style={{ fontWeight: 800, fontSize: "1rem" }}>{item.department}</span>
                    <span style={{ fontSize: "0.75rem", opacity: 0.85 }}>Sem {item.semester}</span>
                  </div>
                  <div>
                ...
                <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                  <button style={{ ...actionBtn, color: "#4f46e5", borderColor: "#c7d2fe" }}
                    onClick={() => setViewItem(item)}>
                    👁 View
                  </button>
                  <button style={{ ...actionBtn, color: "#059669", borderColor: "#a7f3d0" }}
...
const deptBadge = {
  ...
  justifyContent: "center", background: "linear-gradient(135deg,#6366f1,#4f46e5)",
  color: "#fff", borderRadius: 10, padding: "8px 14px", minWidth: 64,
};
const actionBtn = {
...
const tStyle = {
  th: {
    border: "1px solid #e2e8f0", padding: "8px 10px", background: "#f1f5f9",
    fontSize: "0.72rem", fontWeight: 700, whiteSpace: "nowrap", textAlign: "center",
  },
  dayCell: {
    border: "1px solid #e2e8f0", padding: "10px 14px",
    fontWeight: 700, fontSize: "0.82rem", background: "#f8fafc", whiteSpace: "nowrap",
  },
  cell: {
    border: "1px solid #e2e8f0", padding: "10px 8px",
    textAlign: "center", fontSize: "0.75rem", fontWeight: 500,
  },
  breakCell: {
    border: "1px solid #e2e8f0", background: "#fde68a",
    textAlign: "center", fontWeight: 700, fontSize: "0.78rem", padding: "6px 8px",
    color: "#92400e",
  },
  labCell: {
    border: "2px solid #6366f1", padding: "10px 12px",
